...
type
  ...
  PitchEditor* = ref object
    ...
    correctionEditPointSnapStart*: tuple[time: Seconds, pitch: Semitones]
    correctionEditSegmentSnapStart*: tuple[time: Seconds, pitch: Semitones]
...
func resetPitchSnap*(editor: var PitchEditor) =
  if editor.correctionEditIsPoint:
    editor.correctionEditPointSnapStart = (editor.correctionEditPoint.time,
                                           editor.correctionEditPoint.pitch)
...
func handleEditMovement*(editor: var PitchEditor, input: Input) =
  let
    # timeDelta = editor.viewX.scale input.mouseDelta.x
    # pitchDelta = editor.viewY.scale -input.mouseDelta.y
  ...
  template assignPoint(t, p): untyped =
    editor.correctionEditPoint.time = t
    editor.correctionEditPoint.pitch = p
    editor.correctionEditPointCorrection.sort comparePitchPoint

  # template assignSegment(t, p): untyped =
  #   editor.correctionEditSegment.a.time = t
  #   editor.correctionEditSegment.a.pitch = p
  #   editor.correctionEditSegment.b.time = t
  #   editor.correctionEditSegment.b.pitch = p
  ...
  if editor.correctionEditIsPoint:
    if input.isPressed(Shift):
      ...
      let
        pitchDelta = mousePitch - editor.correctionEditPointSnapStart.pitch
        snappedPitch = editor.correctionEditPointSnapStart.pitch + pitchDelta.round
      assignPoint(mouseTime, snappedPitch)
  ...
  #   if not input.isPressed(Shift):
  #     editor.correctionEditSegment.a.pitch = editor.correctionEditSegment.a.pitch.round
  #     editor.correctionEditSegment.b.pitch = editor.correctionEditSegment.b.pitch.round
